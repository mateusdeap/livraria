defmodule LivrariaWeb.SupplierLive.FormComponent do
  ...
  def handle_event("validate", %{"supplier" => supplier_params}, socket) do
    changeset =
      ...
      |> Inventory.change_supplier(supplier_params)
  ...
  defp save_supplier(socket, :edit, supplier_params) do
    case Inventory.update_supplier(socket.assigns.supplier, supplier_params) do
      {:ok, supplier} ->
  ...
  defp save_supplier(socket, :new, supplier_params) do
    case Inventory.create_supplier(supplier_params) do
      {:ok, supplier} ->
        notify_parent({:saved, supplier})
